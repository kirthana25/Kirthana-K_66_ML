...
def create_batches_func(df, Windows, input, output):  
    ## Create X           
...
sess.run(training_op, feed_dict={X: X_batches, y: y_batches})  
        if iters % 150 == 0:  
            ...
            print(iters, "\tMSE:", mse)  
 y_pred = sess.run(outputs, feed_dict={X: X_test}) 
 def build_lstm_layers_func(lstm_sizes, embed, keep_prob_, batch_size):  
    """  
...
def build_accuracy_func(predictions, labels_):  
    """  
    Create accuracy  
    """  
    correct_pred = tf.equal(tf.cast(tf.round(predictions), tf.int32), labels_)  
    accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32))  
    def build_and_train_network_func(lstm_sizes, vocab_size, embed_size, epochs, batch_size,  
